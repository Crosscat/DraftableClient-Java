...
public class CardWebTextureLoader {
    private static Map<String, TextureRegion> textureMap = new HashMap<>();

    public static TextureRegion getTexture(Card card){
        if (textureMap.containsKey(card.name)){
            return textureMap.get(card.name);
        }

        TextureRegion texture = loadTexture(card.image_uris.normal);
        textureMap.put(card.name, texture);
        return texture;
    }

    private static TextureRegion loadTexture(final String url){

        final TextureRegion image = new TextureRegion(new Texture("blankcard.jpg"));
            ...
            public void run() {
                ...
                if (numBytes != 0) {
                    ...
                    Gdx.app.postRunnable(() -> {
                        ...
                        image.setTexture(t);
                        image.setRegion(0, 0, originalWidth, originalHeight);
                    });
                }
            }
        }).start();
